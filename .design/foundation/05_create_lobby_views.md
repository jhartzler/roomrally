...
- **Routing:**
  - `GET /games/:room_code/tv` should route to `games#show_tv`.
  - `GET /games/:room_code/phone` should route to `games#show_phone`.

- **TV View (`games/show_tv.html.erb`):**
...
- **Phone View (`games/show_phone.html.erb`):**
...
## Acceptance Criteria
- Routes and controller actions for the TV and Phone views are created.
- ERB view templates exist for `show_tv` and `show_phone`.
- The TV view displays the game's `room_code`.
- The Phone view for a host contains a disabled "Start Game" button.
- The HTML for both views contains a `<div id="player-list">`.
- The system test is updated to assert the presence of the room code on the TV view and the disabled "Start Game" button on the host's phone view.
