...
- **Broadcasting on Player Join:**
  - When a new `Player` is successfully created (in `players#create`), a broadcast must be sent to the corresponding game channel.
  - This broadcast should be a Turbo Stream that appends the new player's name to the `#player-list` element on all connected clients (TV and Phones).
