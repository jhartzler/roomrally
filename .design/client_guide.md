...
### 1. TV Client (`/games/:room_code/tv`)
...
- **Stimulus Controllers**: `tv_controller` (fullscreen, wake lock), `game_connection_controller`, `timer_controller`.

### 2. Phone Client (`/games/:room_code/phone`)
...
- **Key Views**: Join Game (enter name), Lobby, Prompting (answer form), Waiting, Voting (buttons), Results (your score).
- **Stimulus Controllers**: `phone_controller` (form submission), `game_connection_controller`, `timer_controller`.
...
- `#game_screen`: Main content area on the TV.
- `#phone_screen`: Main content area on the phone.
...
- **`phone_controller` / `tv_controller`**: Handle client-specific interactions (e.g., intercepting a form submission on the phone to send it over the WebSocket).
- **`timer_controller`**: Manages the visual countdown based on data received from the server.

## Routing and Sessions
- `/`: Home page to create or join a game.
- `/games/:room_code`: A splash page to choose between the TV or Phone view.
- `/games/:room_code/tv`: The TV client.
- `/games/:room_code/phone`: The Phone client.
- A `session_id` stored in the Rails session is used to identify a player's browser. If they disconnect and reconnect, this ID allows them to rejoin the game in progress. No user accounts or passwords are required for the MVP.
