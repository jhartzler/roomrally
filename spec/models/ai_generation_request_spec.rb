...
RSpec.describe AiGenerationRequest, type: :model do
  ...
  describe "validations" do
    it "is valid with required attributes" do
      request = build(:ai_generation_request, user:, pack_type: "prompt_pack", pack_id: pack.id, user_theme: "90s movies")
  ...
  describe "#target_pack" do
    it "returns the prompt pack for pack_type prompt_pack" do
      request = create(:ai_generation_request, user:, pack_type: "prompt_pack", pack_id: pack.id)
  ...
  describe "#items_for_indices" do
    ...
    let(:request) { create(:ai_generation_request, user:, parsed_items: items) }
