...
RSpec.describe AiContent::Parser do
  describe ".parse" do
    context "when pack_type is prompt_pack" do
      let(:valid_json) do
        items = 10.times.map { |i| { "body" => "Prompt #{i}" } }
        { "items" => items }.to_json
      end

      it "returns the items array for valid JSON" do # rubocop:disable RSpec/MultipleExpectations
        result = described_class.parse(valid_json, "prompt_pack")
      ...
      it "returns nil when JSON is invalid" do
        expect(described_class.parse("not json", "prompt_pack")).to be_nil
      end

      it "returns nil when items count is not 10" do
        json = { "items" => [ { "body" => "Only one" } ] }.to_json
        expect(described_class.parse(json, "prompt_pack")).to be_nil
      end

      it "returns nil when any item is missing body" do
        items = 10.times.map { |i| i == 5 ? { "other" => "field" } : { "body" => "Prompt #{i}" } }
        expect(described_class.parse({ "items" => items }.to_json, "prompt_pack")).to be_nil
      end
    end

    context "when pack_type is trivia_pack" do
      let(:valid_item) do
      ...
      it "returns items for valid trivia JSON" do
        result = described_class.parse(valid_json, "trivia_pack")
        expect(result).to be_an(Array)
        expect(result.length).to eq(10)
      end
      ...
      it "returns nil when options count is not 4" do
        ...
        expect(described_class.parse(json, "trivia_pack")).to be_nil
      end
    ...
    context "when pack_type is category_pack" do
      let(:valid_json) do
      ...
      it "returns items for valid category JSON" do
        result = described_class.parse(valid_json, "category_pack")
        expect(result).to be_an(Array)
        expect(result.length).to eq(10)
      end
      ...
      it "returns nil when any item is missing name" do
        ...
        expect(described_class.parse({ "items" => items }.to_json, "category_pack")).to be_nil
      end
    end
  end
end
