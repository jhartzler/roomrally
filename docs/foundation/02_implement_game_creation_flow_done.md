...
- **Homepage (`home#index` view):**
  ...
  - Should contain a form to enter a 4-letter code to join an existing room.

- **Room Creation (`rooms#create` action):**
  - The "Create Room" button should POST to `rooms#create`.
  - This action will create a new `Room` record, which automatically gets a unique `code`.
  - After creation, the user should be redirected to the page where they can join as the first player (the host). A good URL for this would be `/rooms/ABCD/join`, where `ABCD` is the new code.

## Acceptance Criteria
- A user can visit the root path of the application.
- The homepage displays a "Create Room" button and a form to enter a code.
